...
@Component({
  ...
  template: `
    <div #container
         [ngClass]="{'ui-multiselect ui-widget ui-state-default ui-corner-all':true,'ui-state-focus':focus,'ui-state-disabled': disabled}"
         [ngStyle]="style" [class]="styleClass"
         (click)="onMouseclick($event,in)">
      <div class="ui-helper-hidden-accessible">
        <input #in type="text" readonly="readonly" [attr.id]="inputId" (focus)="onFocus($event)"
      ...
      <div #panel
           [ngClass]="['ui-multiselect-panel ui-widget ui-widget-content ui-corner-all ui-shadow', panelStyleClass||'']"
           [ngStyle]="panelStyle"
        ...
        <div class="ui-widget-header ui-corner-all ui-multiselect-header ui-helper-clearfix"
          ...
          <div class="ui-chkbox ui-widget" *ngIf="showToggleAll">
            ...
            <div class="ui-chkbox-box ui-widget ui-corner-all ui-state-default"
                 [ngClass]="{'ui-state-active':isAllChecked()}" (click)="toggleAll($event,cb)">
        ...
        <div class="ui-multiselect-items-wrapper">
          <ul class="ui-multiselect-items ui-multiselect-list ui-widget-content ui-widget ui-corner-all ui-helper-reset"
            ...
            <li *ngFor="let option of options; let i = index" class="ui-multiselect-item ui-corner-all"
              ...
              <ng-template [pTemplateWrapper]="itemTemplate" [item]="option" [index]="i"
...
export class MultiSelect implements OnInit, AfterViewInit, AfterContentInit, AfterViewChecked, OnDestroy, ControlValueAccessor {
  ...
  @Input() defaultLabel = 'Column Chooser';
  ...
  @Input() selectedItemsLabel = '{0} Column Chooser';
  ...
  onItemClick(event, value) {
    ...
    if (this.value) {
      for (let i = 0; i < this.value.length; i++) {
        if (this.value[i].display) {
          col++;
        }
        if (this.value[i].field === value.field) {
          this.value[i].display = !this.value[i].display;
          val = this.value[i].display;
          if (val) {
            col++;
          }
          break;
    ...
    this.onChange.emit({ originalEvent: event, value: this.value, index: col, toggle: val });
  ...
  toggleAll(event, checkbox) {
    ...
    if (opts) {
      this.value.forEach(element => {
        ...
        for (let i = 0; i < opts.length; i++) {
          if (opts[i].value.field === element.field) {
            element.display = val;
            break;
          }
        }
    ...
    this.onModelChange(this.value);
    this.onChange.emit({ originalEvent: event, value: this.value, index: -1, toggle: val });
  }
  ...
  isAllChecked() {
    if (this.filterValue && this.filterValue.trim().length) {
      ...
      return this.value && this.options && this.returnVisibleColumns(this.value, this.options);
    }
  }

  /*************************************************************************/
  returnVisibleColumns(value, options) {
    const visibleOptions = value.filter((current) => {
      if (current.display) {
        return current;
      }
    });
  ...
  updateLabel() {
    if (this.value && this.options && this.value.length && this.displaySelectedLabel) {
      ...
      if (this.value.length <= this.maxSelectedLabels) {
        ...
        const pattern = /{(.*?)}/,
          newSelectedItemsLabel = this.selectedItemsLabel.replace(this.selectedItemsLabel.match(pattern)[0], '');
  ...
  onFilter(event) {
    ...
    for (let i = 0; i < this.options.length; i++) {
      const option = this.options[i];
      // if(option.label){
      if (option.label.toLowerCase().indexOf(this.filterValue) > -1 || option.label === '') {
  ...
  isItemVisible(option: SelectItem): boolean {
    if (this.filterValue && this.filterValue.trim().length) {
      for (let i = 0; i < this.visibleOptions.length; i++) {
        if (this.visibleOptions[i].value === option.value && option.label.length > 0) {
          return true;
        }
      }
  ...
  getVisibleOptions(): SelectItem[] {
    if (this.filterValue && this.filterValue.trim().length) {
      ...
      for (let i = 0; i < this.options.length; i++) {
        const option = this.options[i];
        if (option.label.toLowerCase().includes(this.filterValue.toLowerCase()) || option.label === '') {
          items.push(option);
        }
      }
      return items;
