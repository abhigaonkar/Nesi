...
@Component({
    ...
    selector: 'p-listbox',
    template: `
        <div [ngClass]="{'ui-listbox ui-inputtext ui-widget ui-widget-content ui-corner-all':true,'ui-state-disabled':disabled}"
        [ngStyle]="style" [class]="styleClass" (scroll)="onScroll($event)">
            ...
            [ngClass]="{'ui-listbox-header-w-checkbox': checkbox}">
                <div class="ui-chkbox ui-widget" *ngIf="checkbox && multiple && showToggleAll">
                    ...
                    [ngClass]="{'ui-state-active':allChecked}"
            ...
            <ul class="ui-listbox-list">
                <li *ngFor="let option of options; let i = index;" [style.display]="isItemVisible(option) ? 'block' : 'none'"
                    ...
                    (dblclick)="onDoubleClick($event,option)"
...
export class Listbox implements AfterContentInit, ControlValueAccessor {
    ...
    allFilteredSelected(): boolean {
        ...
        if (this.value && this.visibleOptions && this.visibleOptions.length) {
            allSelected = true;
            for (const opt of this.visibleOptions) {
    ...
    toggleAll(event, checkbox) {
        if (this.disabled ||
            (this.filterValue && this.filterValue.trim().length
        ...
        if (checkbox.checked) {
            ...
            const opts = (this.visibleOptions && this.visibleOptions.length) ? this.visibleOptions : this.options;
            if (opts) {
                this.value = [];
