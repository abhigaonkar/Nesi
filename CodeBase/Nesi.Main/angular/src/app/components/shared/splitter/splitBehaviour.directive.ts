...
export class SplitBehaviourDirective implements OnInit {
  ...
  public getElementWidth(): number {
    ...
    const paddingL = parseInt(window.getComputedStyle(this.el.nativeElement, null).getPropertyValue('padding-left'));
    // tslint:disable-next-line:radix
    const paddingR = parseInt(window.getComputedStyle(this.el.nativeElement, null).getPropertyValue('padding-right'));
