...
@Component({
  ...
  selector: 'nesi-timesheetQuoteQuote',
  template: `
  <p-dropdown [disabled]="!loaded || disabled  || !(options && options.length >0)" [style]="{'width':'100%'}" [placeholder]="'&nbsp;'"
  ...
  `,
  providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => TimesheetQuoteQuoteComponent), multi: true }]

})

export class TimesheetQuoteQuoteComponent extends AdvanceValueAccessorBase<any> implements OnInit {
