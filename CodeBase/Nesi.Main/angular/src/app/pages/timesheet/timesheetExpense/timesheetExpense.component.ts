...
@Component({
  ...
  selector: 'nesi-timesheetExpense',
  templateUrl: './timesheetExpense.component.html',
  styleUrls: ['./timesheetExpense.component.css']
})
export class TimesheetExpenseComponent extends TimesheetExpenseBase implements OnInit {
  ...
  get categories(): any[] {
    if (this.selectedUserProfile) {
      return this.selectedUserProfile.categories;
    } else {
      return [];
    }
  }
  ...
  constructor(
    ...
    super(store, cs);
    super.Init(CONFIG.apiURL.page.timesheet.expense.save);
  }
    ...
    this.tss.GetExpenseCurrentUserProfile()
      .subscribe(
        (res: ExpenseCurrentProfile) => {
          ...
          //if(this.currentUserProfile.currency!=null && this.currentUserProfile.currency>=1)
          this.userform.get('currency').setValue(this.currentUserProfile.currency);
  ...
  createForm() {
    this.userform = this.fb.group({
      ...
      'wo_number': '',
      'isShop': false,
      'pre_tax_amount': ['', [Validators.required, Validators.min(0.01)]],
  ...
  loadSelectedUserProfile() {
    this.tss.GetExpenseSelectedUserProfile(this.userform.get('id_member').value)
  ...
  categoryChanged(event) {
    const selectUserId = this.userform.get('id_member').value;
...
workorderChanged(event: any)
{
    const selectworkorderNum=this.userform.get('wo_number').value;
    super.LOG(CONFIG.apiURL.page.timesheet.expense.currency+ '/' + selectworkorderNum.toString(), 'currency response');
    this.cs.getObject<any>(CONFIG.apiURL.page.timesheet.expense.currency+ '/' + selectworkorderNum.toString())
    .subscribe(
      (res) => {
         super.LOG(res, 'currency response');
         if(res!=null && res>=1)
         {
             this.Currency=res-1;
             this.userform.get('currency').setValue(this.Currency); 
         }
      },
      (err:any)=>
      {
  ...
  formValidateBefore() {
    ...
    if (!(this.selectedUserProfile && this.selectedUserProfile.categories)) {
      return;
    }
    const selected = super.selectedItem(this.selectedUserProfile.categories, 'value', categoryId);
    if (selected) {
