...
export class NesiR2Component implements OnInit, AfterViewInit {
  ...
  isReleaseMenu(id: number): boolean {
    ...
    if (item && !item.hide_badge && !(this.isMobile && item.mobileN1Link)) {
      this.router.navigate([item.routerLink]);
      return true;
    } else {
      return false;
