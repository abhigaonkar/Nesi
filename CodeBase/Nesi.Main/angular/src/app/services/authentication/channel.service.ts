...
export class ChannelService {
    ...
    sub(channel: string): Observable<ChannelEvent> {
        ...
        this.starting$.subscribe(() => {
            this.hubProxy.invoke('Subscribe', channel)
                .done(() => {
                    CONFIG.LOG(`Successfully subscribed to ${channel} channel`, 'subscribe hub');
                })
                .fail((error: any) => {
    ...
    publish(ev: ChannelEvent): void {
        const hub = this.hubConnection.createHubProxy(CONFIG.SignalR.hubName);
        hub.invoke('Publish', ev)
            .done(res => { CONFIG.LOG(res, 'firesingalRevent') })
            .fail(err => { CONFIG.LOG(err, 'firesingalRevent') });;
    }
