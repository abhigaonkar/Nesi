...
public partial class sections_reports_workorder_preview_frame : System.Web.UI.Page
	{
	...
	protected void Page_Init(object _sender, EventArgs _e)
		...
		_approve_dm = _current_user.AuthenticatedForPrivilege(191);
		...
		Title                   = "Work Order Preview for WO " + wo.OrderNumber.TrimStart('0');
				...
				try
					...
					if (_approve_bm || _approve_dm && (wo.business_unit_id == _current_user.business_unit_id || temp_parent_wo.business_unit_id == _current_user.business_unit_id))
						...
						if (_is_child_approval && child_wo.parent_woprog_id > 1)
							...
							wo_dc.description   = string.Format("WO-{0}({1}) materials", child_wo.OrderNumber, child_company.name);
							wo_dc.master_id     = OpsSpecialPart.NewChildWO;
							if (child_wo.woprog_associate_woprog_id != 0)
								{
								wo_dc.description = string.Format("WO-{0}({1}) progress bill on Q{2}", child_wo.OrderNumber, child_company.name, child_wo.QuoteID);
								wo_dc.cost = child_wo.woprog_StillToBeBilled;
								}
							else if (child_wo.QuoteID != "0") // if its a quote, grab the whole amount
								{
								wo_dc.description = string.Format("WO-{0}({1}) Quote Q{2}", child_wo.OrderNumber, child_company.name, child_wo.QuoteID);
							...
							wo_dc.sell          = wo.use_fixed_material_markup? wo_dc.cost*wo.fixed_material_markup : shared.GetSellPrice(wo_dc.cost, 0, false, 1, wo_dc.business_unit_id);
							wo_dc.unit          = wo.use_fixed_material_markup ? wo_dc.cost * wo.fixed_material_markup : shared.GetSellPrice(wo_dc.cost, 0, false, 1, wo_dc.business_unit_id);
