...
public partial class sections_reports_invoice_preview_index : System.Web.UI.Page
	{
	int wo_id;
	bool is_signoff;
	...
	protected void Page_Load(object sender, EventArgs e)
		...
		if (wo_id > 0)
			...
			Title = is_signoff 
						? "Work Order Acceptance for " + this_woprog.OrderNumber.TrimStart('0') 
						: "Draft " + this_woprog.OrderNumber.TrimStart('0');
			...
			if (!IsPostBack)
				...
				memoBody.Text += $"Please review the attached acceptance form.\n\n Please don't hesitate to call us at {company.PhoneNumber}.\n\n";
				}
			...
			var report = new InvoicePreview(wo_id, breakout);
			InvoicePreviewer.Report = (InvoicePreview)fill_report(report);
			InvoicePreviewer.Report.Name = "Signoff for WO " + wo_id;
			...
			if (is_pdf)
				...
				report.ExportToPdf($@"{fileServer}\wos\{this_woprog.woprog_id}.pdf", pdfOptions);
	...
	protected object fill_report(InvoicePreview report)
		{
		...
		var invoice_number_title = report.FindControl("lbl_invoice_num", true) as XRLabel;
		...
		invoice_number.Visible = false;
		invoice_number_title.Visible = false;
		// Check for signature
		...
		try
			...
			if (this_woprog.woprog_Address_ID != this_customer.Address.id)
				{
				var address = new NEAddress(this_woprog.woprog_Address_ID);
...
{1},{2},{3}",
		...
		if (is_signoff)
			...
			attn_name.Text = "";
			lblattn.Text = "";
			inv_header_label.Text = "Sign Off Acceptance";
			var work_completed = "";
		...
		if (this_woprog.Status == "Invoiced" && string.IsNullOrEmpty(netsuite_id))
			{
...
{7}",
					...
					tax_number,
					this_company.web_domain
					);
			inv_header_label.Text = "Draft ";
	...
	protected void btnSendEmail_Click(object sender, EventArgs e)
		...
		this_woprog = new NeWOProg(wo_id);
		var report = new InvoicePreview(wo_id, breakout);
		report = (InvoicePreview)fill_report(report);
		...
		var _type = "Draft Emailed";
		report.Watermark.Text = "Not An Invoice";
		if (is_signoff)
			{
			mail.Attachment = new Attachment(file, "Work Order Acceptance " + this_woprog.OrderNumber + ".pdf");
			_type = "Work Order Acceptance Emailed";
			}
		else
			{
			mail.Attachment = new Attachment(file, "Draft " + this_woprog.woprog_id + ".pdf");
			}
		...
		ScriptManager.RegisterStartupScript(this, this.GetType(), "Error", "alert('Email Sent');", true);
