...
public partial class sections_reports_wo_line_grid_index : Page
	{
	Toolbox _tools;
	NeMember current_user;
	JavaScriptSerializer jSON		= new JavaScriptSerializer();
	static int _page_id			= 105;
	static string _page_name		= "WOLineGrid";
	ASPxHiddenField h;
	...
	protected void Page_Init(object sender, EventArgs e)
		...
		layout.used_gv			= gv_wo_line_grid;
		...
		if (!current_user.AuthenticatedForPrivilege(93))
		{
			gv_wo_line_grid.Columns.Remove(gv_wo_line_grid.Columns["cost"]);
            gv_wo_line_grid.Columns.Remove(gv_wo_line_grid.Columns["total_cost"]);
		}
		}
	protected void Page_Load(object sender, EventArgs e)
		{
			var lbltemp = (Label)Page.Master.FindControl("lblHeading");
			lbltemp.Text = "WO Line Grid";

            
		    gv_wo_line_grid.DataSource = Toolbox.doSQL_dt(@"CALL report_wo_line_grid(@v0)",new object[] { current_user.id } );
		if(!IsCallback && !IsPostBack)
			{
			var menu			= new NeMenu(current_user, Convert.ToInt32(_page_id));
			divMenu.InnerHtml	= menu.MenuHTML;
			var gl	= new NeGridLayouts(current_user.id, _page_name);
			h.Set("gridview_id", "gv_wo_line_grid");
			if(gl.GridLayoutID == 0)
				{
                    gv_wo_line_grid.FilterExpression = string.Format("[business_unit] = '{0}' AND [PM] = '{1}' AND [wo_status] = 'Open' AND [qty_unfulfilled] > 0", current_user.business_unit.name, current_user.FirstName);
				gl.GridLayout_Layout				= gv_wo_line_grid.SaveClientLayout();
				...
				{
				gv_wo_line_grid.LoadClientLayout(gl.GridLayout_Layout);
				h.Set("ID", gl.GridLayoutID);
				h.Set("NAME", gl.GridLayout_Name);
				}
	...
	protected void gv_wo_line_grid_HtmlDataCellPrepared(object sender, ASPxGridViewTableDataCellEventArgs e)
	{
	}
