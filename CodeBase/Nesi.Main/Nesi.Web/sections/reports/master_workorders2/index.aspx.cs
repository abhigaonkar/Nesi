...
public partial class master_workorders2 : Page
	{

	NeMember myMember;
	private const int _page_id = 193; // from Page table in DB
	private const string _page_name = "MasterWorkOrderV2";
	...
	protected void Page_Init(object sender, EventArgs e)
		...
		var char_count		= myMember.FullName.Length + myMember.business_unit.name.Length+ 62;
		default_filter = string.Format("page1|sort1|d8|filter186|[Cut_Date] > #2014-01-01# And [Branch] In ('Barrie', 'Brampton', 'Cambridge', 'Charlotte', 'Fresno', 'Mississauga', 'Oakville', 'Plymouth', 'Sterling Heights', 'Stoney Creek', 'Vaughan')|conditions12|6|10|8|8|9|8|10|8|13|9|14|4|15|5|19|9|23|6|24|5|26|5|41|9|visible44|t0|t4|f7|t1|t2|f9|f24|t20|t19|t16|t14|t13|f25|f10|f21|t15|f3|f26|t12|f5|t17|f27|f28|f11|f29|f8|f30|f31|f32|t18|f33|f34|f35|f36|f23|f22|f37|f37|f37|f75|f48|f50|f52|t78|width44|61px|107px|50px|91px|100px|100%|25px|122px|146px|64px|89px|75px|50px|50px|25px|89px|20px|25px|103px|49px|113px|20px|20px|30px|129px|89px|30px|30px|40px|114px|25px|40px|40px|e|124px|116px|e|e|40px|150px|75px|60px|50px|108px", myMember.FirstName, myMember.business_unit_name, char_count);
		...
		if (Cache["ds2_depend"] == null)
			{
			Cache["ds2_depend"] = DateTime.Now;
			}
		view_margin = myMember.AuthenticatedForPrivilege(58);
		view_dollar_totals			= myMember.AuthenticatedForPrivilege(59);
		view_total_time				= myMember.AuthenticatedForPrivilege(60);
	...
	protected void Page_Load(object sender, EventArgs e)
		...
		if (!IsCallback && !IsPostBack)
			...
			var branch_filter = !string.IsNullOrEmpty(_q["business_unit_id"]) ? string.Format("[Branch] = '{0}'", temp_company.name) : "";
			var status = !string.IsNullOrEmpty(_q["status"]) ? string.Format(" [Status] = '{0}'", _tools.value_from(_q["status"], false)) : "";
			if (gl.GridLayoutID != 0 && _q["business_unit_id"] == null)
				...
				gv_workorders.LoadClientLayout(string.Format("page1|sort1|d8|filter186|[Cut_Date] > #2014-01-01# And [Branch] In ('Barrie', 'Brampton', 'Cambridge', 'Charlotte', 'Fresno', 'Mississauga', 'Oakville', 'Plymouth', 'Sterling Heights', 'Stoney Creek', 'Vaughan')|conditions12|6|10|8|8|9|8|10|8|13|9|14|4|15|5|19|9|23|6|24|5|26|5|41|9|visible44|t0|t4|f7|t1|t2|f9|f24|t20|t19|t16|t14|t13|f25|f10|f21|t15|f3|f26|t12|f5|t17|f27|f28|f11|f29|f8|f30|f31|f32|t18|f33|f34|f35|f36|f23|f22|f37|f37|f37|f75|f48|f50|f52|t78|width44|61px|107px|50px|91px|100px|100%|25px|122px|146px|64px|89px|75px|50px|50px|25px|89px|20px|25px|103px|49px|113px|20px|20px|30px|129px|89px|30px|30px|40px|114px|25px|40px|40px|e|124px|116px|e|e|40px|150px|75px|60px|50px|108px", branch_filter, status, char_count));
	...
	protected void populate_grid()
		...
		if (Session["master_wo2_gv"] == null)
			{
				Session["itotal"] = 0;
				Session["irows"] = 0;
			var chk		= chk_includeprogress.Checked ? 1 : 0;
            var dt = Toolbox.doSQL_dt(@"call report_wo_master_v2(" + view_dollar_totals + "," + view_margin + ",'" + new Current_User().visible_business_units + "')",null);
			Session["master_wo2_gv"] = dt;
			}

		gv_workorders.DataSource = Session["master_wo2_gv"];
	...
	protected void gv_workorders_CommandButtonInitialize(object sender, ASPxGridViewCommandButtonEventArgs e)
	    ...
	    protected void cb_exp_hours_Callback(object source, DevExpress.Web.CallbackEventArgs e)
	        ...
	        _tools.getSQL_void(@"UPDATE woprog SET woprog_exp_labor = @v0  WHERE woprog_id = @v1  LIMIT 1", new object[] { w.exp_hours, w.woprog_id });
	        if (Session["master_wo2_gv"] != null)
	            {
	            DataTable dt = (DataTable)Session["master_wo2_gv"];
	...
	protected void cb_expected_date_Callback(object source, DevExpress.Web.CallbackEventArgs e)
		...
		_tools.getSQL_void(@"UPDATE woprog SET woprog_expected_enddate = @v0  WHERE woprog_id = @v1  LIMIT 1", new object[] {  Toolbox.MySQL_shortdt(w.end_date), w.woprog_id } );
		    if (Session["master_wo2_gv"] != null)
		        {
		        DataTable dt = (DataTable)Session["master_wo2_gv"];
		        DataRow dr = dt.Select("woprog_id=" + w.woprog_id)[0];
	...
	protected void cb_Callback(object sender, DevExpress.Web.CallbackEventArgsBase e)
		...
		try
			...
			if (p[2] == "n")
				...
				if (_tools.getSQL_affectedrows(@"Update woprog_project_notes  set woprog_project_notes_notes=@v0  where woprog_project_notes_woprogid=@v1 and woprog_project_notes_Type = 'W' limit 1", 
					new object[] { p[1],p[0] }) == 0)
					{
					_tools.getSQL_affectedrows(@"Insert into woprog_project_notes (woprog_project_notes_notes,woprog_project_notes_woprogid,woprog_project_notes_Type,woprog_project_notes_datetime,
woprog_project_notes_memberid)  values (@v0,@v1,'W',curdate(),@v2)",
new object[] { p[1],p[0],myMember.id } );
	...
	public static void update_lbr(double hrs, int woprog_id)
		...
		if(woprog_id > 10000 && hrs >= 0)
			...
			HttpContext.Current.Session["master_wo2_gv"] = null;
			}
		else if(hrs < 0)
			{
