...
using bllT = NESI.BLL.Pages.Reports.CustomerSurveysGrid;
using dtoT = NESI.DTO.ViewModels.Page.Reports.CustomerSurveysGrid;

namespace NESI.WebAPI.Controllers.API.Page.Reports
{
    [PageAuthorizationFilter(161)]
    [RoutePrefix("api/Page/CustomerSurveysGrid")]
    public class CustomerSurveysGridController : EmployeeGridControllerBase<dtoT, bllT>
    {
        public CustomerSurveysGridController()
        {
            key = "id";
            moduleName = "CustomerSurveysGrid";
        }

        protected override bllT GetObject(BodyParams param, params object[] extra_params)
        {
            param.keyColumn = key;
            return new bllT(CurrentEmployee, param) { SetColumnList = param.columns };
        }
